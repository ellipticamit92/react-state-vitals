...
function buildStore(name, initializer, limitKB) {
  const snapshots = [];
  let renderCount = 0;
  const store = zustandCreate(initializer);
...
function devtools(initializer, options) {
  ...
  return zustandDevtools(initializer, options);
}
function persist(initializer, options) {
  setNameContext(options.name);
  return zustandPersist(initializer, options);
}
...
//# sourceMappingURL=chunk-HELXSMSG.mjs.map